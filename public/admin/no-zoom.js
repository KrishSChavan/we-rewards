/* Zoom off — matches the student PWA and vendor terminal so the three installed
   apps behave the same. Three layers, because no single one covers every
   browser:

     1. <meta viewport user-scalable=no>  — Android Chrome, and iOS once the
        PWA is installed (standalone mode honours it).
     2. touch-action: pan-x pan-y on <html> (see admin.css) — kills pinch and
...
(function () {
  ...
  // affordance, and matters most on this one, since the dashboard is mostly
  // read on a desktop.
