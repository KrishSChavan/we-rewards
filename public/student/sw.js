...
const CACHE = 'werewards-v86';   // v86: a visit lands in the RECENT SPOTS row on the socket push that carries it, instead of waiting for the next /balances refetch. app.js only — no markup changed — but it is precached, so an installed PWA would keep the old handlers and go on ignoring the server's new `visit` flag until something else happened to refetch
