...
const CACHE = 'werewards-v90';   // v90: Sign in always shows Google's account chooser (prompt=select_account), so signing out and back in can reach a DIFFERENT account on the same device instead of silently re-entering the last one. app.js only — but skipping this bump is what makes the fix invisible to installed PWAs, which is precisely where someone juggling a personal and a student account is stuck
