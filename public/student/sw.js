...
const CACHE = 'werewards-v88';   // v88: a crash on a spot screen reports WHICH spot, so the operator's error log can name it instead of showing an anonymous "Student app · spot". app.js only. The cost of skipping this bump is invisible rather than broken — an installed PWA keeps posting crash reports with no vendor in them, and the new Vendor row in /admin stays empty for exactly the students whose app is installed, which is most of them
...
// v85: the ambassador QR sheet (migration-053). index.html gained the Account button and the sheet markup, and app.js/styles.css gained everything behind them — all three are precached, so without this bump an installed PWA keeps the old shell and an ambassador sees no button at all while the server happily answers /api/me/ambassador
