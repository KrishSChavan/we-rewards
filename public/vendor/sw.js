...
// (index.html + terminal.js + terminal.css).
const CACHE = 'werewards-terminal-v36';   // v36: the billing card tells a vendor who cancelled that their plan RUNS OUT on the renewal date rather than that they will be charged again on it. cancelAtPeriodEnd is read live from Stripe in GET /api/vendor/billing (migration-055 has no column for it). terminal.js only.
// v33: the ITEMS toggle swaps its ON/OFF label for a spinner until the server confirms, and an error line under the list says why when it doesn't stick. terminal.js, terminal.css and index.html (the new #reward-error paragraph) all changed together and all three are precached — without this bump an installed counter iPad runs the new terminal.js against a shell that has no #reward-error in it, so every failed toggle throws on the missing element instead of explaining itself
