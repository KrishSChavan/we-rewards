...
// (index.html + terminal.js + terminal.css).
const CACHE = 'werewards-terminal-v37';   // v37: the SETTINGS plan & billing region is hidden outright on a deployment with no Stripe keys, unless the vendor has a billing relationship or an outstanding payment. Without this, production (which ships with no keys) advertises an upgrade nobody can buy. terminal.js only.
